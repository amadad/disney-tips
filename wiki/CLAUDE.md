...
Domain: Walt Disney World and Disneyland trip planning.

Goal: feed a paid Disney trip-planner product. A user fills out a short
intake form (dates, party, budget, priorities) and receives a personalized
trip book. The book is generated by an LLM call that reads this wiki. The
wiki's job is to make that generation fast, grounded, and current — and to
compound knowledge across sessions so the product gets richer as more
sources are ingested and more queries are run.
...
- **Summaries of raw sources** (`./sources/`) — one per ingested source,
  ...
  ingest notes, not the primary reader-facing trip-planning pages.
...
- **Strategy pages** (`strategy-` filename prefix) — how to use things
  well. Separated from entities so the trip planner can retrieve them
...
The wiki's primary consumer is an agent (the trip-planner LLM call, or
the operator asking questions) that reads pages to produce an answer.
...
decks, matplotlib charts, structured trip books. **Good answers get
...
- Coverage gaps — entities the trip planner would want to retrieve but
  no page exists
...
and vice versa. Rationale: a paid trip planner retrieves strategy
